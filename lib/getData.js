...
var getCurrentOptions = function (cb) {
  ...
  try {
    if (fs.statSync(yml).isFile()) {
      ...
      options.domain_name = qualitywatcher_yaml_conf.domain_name === undefined ? 'app' : qualitywatcher_yaml_conf.domain_name;
...
function gitData(git, cb) {
  ...
  child.exec("git rev-parse --verify " + git.head.id, function (err, response) {
    if (err) {
...
function fetchBranch(git, cb) {
  child.exec("git branch", function (err, branches) {
    if (err) {
      return cb(err);
    }
...
function fetchHeadDetails(git, cb) {
  child.exec('git cat-file -p ' + git.head.id, function (err, response) {
    if (err) {
...
function fetchRemotes(git, cb) {
  child.exec("git remote -v", function (err, remotes) {
    if (err) {
      return cb(err);
    }
